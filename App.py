...
class Assets(object):
    ...
    def __init__(self):
        ...
        self.images["UI"] = (
            ...
            pygame.image.load(
                'assets/picture/settingmenu.png').convert_alpha(),
        ...
        self.images["ELEMENTS"] = (
            ...
            pygame.transform.scale(pygame.image.load(
                'assets/picture/yes.png').convert_alpha(), (45, 45)),
            pygame.transform.scale(pygame.image.load(
                'assets/picture/no.png').convert_alpha(), (45, 45)),
        ...
        self.sounds["pickheart"] = pygame.mixer.Sound(
            'assets/sounds/pickheart.ogg')
...
class Barrier(object):
    ...
    def spawn(self):
        if self.ground and self._groundlvl <= self._threshold:  # 在地面 权重不达标
            barrier = GSprite(
                self.screen.assets.images["ELEMENTS"][random.randint(8, 9)])
            barrier.position = (1600, 800-barrier.rect.height)
            self.barrier_group.add(barrier)

            barrier = GSprite(
                self.screen.assets.images["ELEMENTS"][random.randint(5, 6)])
...
class Player(object):
    def __init__(self, screen):
        ...
        # 玩家初始化
        self.player = GSprite(
            screen.assets.images["ELEMENTS"][0], speed=self.speed)
    ...
    def Action(self):
        ...
        if self.player.rect.top < 100 or self.player.rect.bottom > self.screen.height - 100:
            if abs(self.speed) > 4:
                ...
                self.speed = 0
        self.player.rect.top = min(
            max(self.player.rect.top, 100), self.screen.height-100)
        self.player.rect.bottom = min(
            max(self.player.rect.bottom, 100), self.screen.height-100)
        if self.health == 0:
...
class GameBackground(object):
    ...
    def __init__(self, screen):
        ...
        self.bg1 = pygame.transform.scale(
            pygame.image.load(
                self.screen.assets.background_list[random.randint(
                    0, len(self.screen.assets.background_list)-1)]
    ...
    def drawscore(self):
        if self.screen.player.score == 0:
            self.screen.scene.blit(
                self.screen.assets.images["NUM"][0], (1503, 36))
        ...
        for v in score:
            if v == 1:
                ...
                x -= 34
            self.screen.scene.blit(
                self.screen.assets.images["NUM"][int(v)], (x, 36))
    ...
    def drawheart(self):
        ...
        if health > 0:
            ...
            if full != 0:
                for i in range(full):
                    self.screen.scene.blit(
                        self.screen.assets.images["ELEMENTS"][3], (40+i*67, 28))
            else:
                i = -1
            if half != 0:
                self.screen.scene.blit(
                    self.screen.assets.images["ELEMENTS"][4], (40+(i+1)*67, 28))

    def DrawUI(self):
        if self.uipart <= 1:
            self.screen.scene.blit(
                self.screen.assets.images["UI"][self.uipart], (0, 0))
        else:
            self.screen.scene.fill((0, 0, 0))
        if self.uipart == 1:  # SettingMenu
            if self.screen.config.config["BackgroundMusic"]:
                self.screen.scene.blit(
                    self.screen.assets.images["ELEMENTS"][1], (356, 234))
            else:
                self.screen.scene.blit(
                    self.screen.assets.images["ELEMENTS"][2], (303, 234))
            if self.screen.config.config["BackgroundSoundEffect"]:
                self.screen.scene.blit(
                    self.screen.assets.images["ELEMENTS"][1], (356, 367))
            else:
                self.screen.scene.blit(
                    self.screen.assets.images["ELEMENTS"][2], (303, 367))
